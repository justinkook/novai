...
const realSettingsApi = {
  ...
  inviteUser: async (data: InviteUserData) => {
    const response = await apiCall<InviteResponse>(
      '/tenants/invite',
      'POST',
      data
    );
  ...
  validateInvitation: async (token: string) => {
    const response = await apiCall<{ message: string }>(
      `/tenants/validate-invitation/${token}`
    );
    return response.message;
  },
  ...
  acceptInvitation: async (token: string) => {
    const response = await apiCall<{ message: string }>(
      `/tenants/accept-invitation/${token}`,
      'POST'
    );
    return response.message;
  },
  ...
  cancelInvitation: async (invitationId: string) => {
    const response = await apiCall<{ message: string }>(
      `/tenants/invitations/${invitationId}`,
      'DELETE'
    );
    return response.message;
  },
