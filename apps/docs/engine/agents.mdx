...
NovAI uses LangGraph to orchestrate multi-step agent workflows (graphs) such as Open Canvas, gameplay engine, reflection, summarizer, thread titles, and web search.
...
```json
{
  ...
  "graphs": {
    ...
    "game_engine": "./apps/agents/src/game-engine/index.ts:graph"
  },
  "env": ".env"
}
```
...
- Agents: `apps/agents/.env` (server-only secrets)
  ...
  - Optional providers: `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `FIREWORKS_API_KEY`, `GROQ_API_KEY`, `EXA_API_KEY`
