...
const llmConfig: LLMConfig = {
  ...
  model: 'llama2:7b' // or other local models
};
```
...
class CustomLLMProvider implements LLMProvider {
  async call(prompt: string, memory: string[], temperature: number): Promise<string> {
...
class SecureLLMService extends LLMService {
  ...
  async callLLM(prompt: string, memory: string[], temperature: number): Promise<string> {
    console.log(`LLM call to ${this.config.provider} with temperature ${temperature}`);
