...
export const buildPrompt = ({
  ...
  return UPDATE_ENTIRE_ARTIFACT_PROMPT.replace(
    ...
    .replace('{engineResultsContext}', engineResultsContext || '')
    .replaceAll('{characterName}', characterName || 'Tav')
...
export const createNewArtifactContent = ({
  ...
  const baseContent = {
    index: (state.artifact?.contents?.length || 0) + 1,
