...
function isSummaryMessage(msg: unknown): boolean {
  ...
  if (
    ...
    (msg as { additional_kwargs?: AnyKeyRecord }).additional_kwargs?.[
  ...
  if (
    'kwargs' in msg &&
    (msg as { kwargs?: { additional_kwargs?: AnyKeyRecord } }).kwargs
...
export const NovaiGraphAnnotation = Annotation.Root({
  /**
  ...
  /**
   * BG3-specific: Active session id
   */
  sessionId: Annotation<string | undefined>,
  /**
   * BG3-specific: flag to save current artifact as chapter
   */
  ...
  /**
   * BG3-specific: engine output for latest turn
   */
  output: Annotation<
    | {
        narration: string;
...
export type NovaiGraphState = typeof NovaiGraphAnnotation.State;
export type NovaiGraphReturnType = Partial<typeof NovaiGraphAnnotation.State>;
